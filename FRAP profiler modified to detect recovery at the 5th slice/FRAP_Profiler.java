...
//Also asks for single or double exponential curve fit.
		...
		//fit curve
		// For this FRAP experiment the fit is hardwired to start at the slice #5 (the first scan after photobleaching is finished). Slices are counted from 0-i. 
			int sliceMin=4;
			    //The original layout for automated detection of the slice after photobleaching is commented out
			    //for (int i=0;i<y.length; i++)
				//{
				//We'll find the Y minimum, and assume that this is the first post-bleach point...
				//if(y[i]==ymin) sliceMin=i+1;	
				//}
			//The subtraction of one more slice is commented out. Assuming it was designed to compensate for recovery during photobleaching, it is not needed with FRAP settings of this expereiment (time of photobleaching (0.1 sec) is neglegtable in comparison to the scanning speed (1 slice/0.743 sec)).
			//sliceMin=sliceMin - 1;
